...
void main () {
  vec4 actual_pos = vec4(position.xyz + translation.xyz, 1.0);
  ADS(actual_pos, normal, vec4(light_pos,1.0), normal_matrix, modelview_matrix, N, V, L);
  frag_text_coord = texture_coord;
  eye_position    = modelview_matrix * actual_pos;
  world_position  = model_matrix * actual_pos;
