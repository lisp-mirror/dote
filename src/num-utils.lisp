  ...
  (let ((fun-name (alexandria:format-symbol t "~:@(~a-interpolate~)" name)))
    `(progn
	 ...
	 ,(alexandria:with-gensyms (w)
	   `(let ((,w (alexandria:clamp (d/ (d- weight a) (d- b a)) (desired 0)
					(desired 1))))
	      (,interpolator-fun ,w))))
...
(defun clamp-0->max-less-one (val max)
  (alexandria:clamp val (desired 0.0) (desired (1- max))))
  ...
  (let ((sigma-xy (d* sigma-x sigma-y))
	...
	(exp (d* (d- (d/ (desired 1) (d* (desired 2) (d- (desired 1)
		 ...
		 (d+ (d/ (expt (d- x mu-x) (desired 2)) (expt sigma-x (desired 2)))
		     ...
		     (d- (d/ (d* (desired 2) correlation (d- x mu-x) (d- y mu-y)) sigma-xy))))))))

(defun gaussian (x sigma &optional (mu 0))
  (declare (optimize (debug 3) (safety 0) (speed 3)))
  (d* (d/ (desired 1) (d* sigma (sqrt (d* (desired 2) constants:+pi+))))
