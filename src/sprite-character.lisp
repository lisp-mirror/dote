...
(alexandria:define-constant +default-animation-table+
    '((:stand        ((8  2 0.1)
      ...
      (:melee        ((12 6 0.3)
                      ...
                      (14 6 0.3)
                      (15 6 0.3)))
      (:long-range   ((16 10 0.3)
                      (17 10 0.3)
                      (18 10 0.3)
                      (19 10 0.3)))
      (:attack-spell ((0 7 0.1)
                      (1 7 0.1)
                      (2 7 0.1)
                      (3 7 0.1)
                      (4 7 0.1)))
  ...
  (with-accessors ((state state)
    ...
    (labels ((stop-movements (already-stopped interrupt-if-ai interrupting-id)
             ...
             (seenp (entity &key
               ...
               (when (not (thrown-down-in-fow-p object))
  ...
  (with-accessors ((state                   state)
    ...
    (game-state:with-world (world state)
      (let* ((camera      (world:camera world))
             (camera-dir  (let ((diff (vec- (camera:target camera)
                                            (pos camera))))
                            (setf (vec-y diff) 0.0)
                            (normalize diff)))
        ...
        (flet ((%set-animation (index-dir)
                 ...
                 (setf animation-speed        (animation-row-speed anim-spec index-dir))))
...
(defun animation-row-speed (anim-spec-row facing)
  (let ((all (cadr anim-spec-row)))
    (caddr (elt all facing))))
