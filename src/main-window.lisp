        ...
        (when (string= text "P")
          (let ((camera (world:camera (world object))))
            (misc:dbg "~a ~a
                      ...
                      (entity:up     camera)
                      (entity:dir    camera))))
        (when (find text (list (gconf:forward)
                               (gconf:back) (gconf:left) (gconf:right)
                               (gconf:upward) (gconf:downward)
                               (gconf:go-to-active-player)
                               "o" "i" "u" "d" "V" "B"
                               "n" "N" "V" "v" "U" "D"
                               "L" "p")
                    :test #'string=)
          (when (string= text (gconf:forward))
            (slide-forward (world object)))
          (when (string= text (gconf:back))
            (slide-back (world object)))
          (when (string= text (gconf:left))
            (slide-left (world object)))
          (when (string= text (gconf:right))
            (slide-right (world object)))
          (when (string= text (gconf:upward))
            (slide-upward (world object)))
          (when (string= text (gconf:downward))
            (slide-downward (world object)))
          (when (string= text (gconf:go-to-active-player))
