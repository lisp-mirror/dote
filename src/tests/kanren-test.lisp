...
(defun make-tactics-3-3 ()
  (let* ((def (list (list (cons (ivec2 4 0) nil)
                          (cons (ivec2 3 0) nil)
                          (cons (ivec2 7 0) nil))))
         (atk (list (cons (ivec2 10 0) 7)
                    ...
                    (cons (ivec2 21 0) 31)))
         (res (car (run* (q)
                     (make-attack-tactics def 3 atk q)))))
    (loop for i in res
...
(defun make-tactics-2-2 ()
  (let* ((def (list (cons (ivec2 3 0) nil)
         ...
         (atk (list (cons (ivec2 10 0) 7)
                    (cons (ivec2 20 0) 30)))
         (res (first (run* (q)
                       (make-attack-tactics (list def) (length def) atk q)))))
    (loop for i in res
         when (every #'(lambda (a) (not (null (cdr a)))) i)
       collect i)))
  ...
  (let ((*reachable-p-fn* #'%reachablep))
    ...
    (assert-true
        (%comp-tactics (make-tactics-3-2)
                       '(((#(4 0) #(10 0) 7) (#(2 0)) (#(7 0))) ((#(4 0)) (#(2 0)) (#(7 0)))
                         ((#(4 0)) (#(2 0)) (#(7 0) #(10 0) 7)) ((#(4 0) #(10 0) 7) (#(2 0)) (#(7 0)))
