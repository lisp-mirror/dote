...
(defmethod rendering-needed-p ((object terrain-chunk) renderer)
  ...
  (let* ((results   (copy-flat object))
    ...
    (setf (lookup-tile-triangle results)
          (matrix:submatrix (lookup-tile-triangle results)
                            (map-utils:coord-chunk->matrix (aabb2-min-x aabb))
                            (map-utils:coord-chunk->matrix (aabb2-min-y aabb))
                            (f- (map-utils:coord-chunk->matrix (aabb2-max-x aabb))
                                (map-utils:coord-chunk->matrix (aabb2-min-x aabb)))
                            (f- (map-utils:coord-chunk->matrix (aabb2-max-y aabb))
                                (map-utils:coord-chunk->matrix (aabb2-min-y aabb)))))
...
(defmethod triangle-matrix-elt ((object terrain-chunk) row col)
  (with-accessors ((aabb aabb) (triangle triangles)) object
    (let* ((min-x  (min-x aabb))
           ...
           (max-x  (max-x aabb))
           (max-z  (max-z aabb))
           (width  (truncate (map-utils:coord-chunk->matrix (d- max-x min-x))))
           (height (truncate (map-utils:coord-chunk->matrix (d- max-z min-z)))))
...
(defmethod approx-terrain-height ((object terrain-chunk) x z)
  ...
  (with-accessors ((lookup-tile-triangle lookup-tile-triangle)) object
    (let* ((x-cost (map-utils:coord-chunk->costs x))
           (y-cost (map-utils:coord-chunk->costs z))
